...
CREATE OR REPLACE FUNCTION public.update_director_tecnico_order(
    ...
    p_es_reinspeccion boolean DEFAULT false -- 👈 NUEVO PARÁMETRO
)
...
BEGIN
    ...
    -- =========================================================================
    -- PASO 2.6: DESCUENTO CONDICIONAL DE CUPOS (FUPAS Y CERTIFICADOS)
    -- =========================================================================
    -- 1. Obtenemos el tenant_id asociado a esta orden
    SELECT tenant_id INTO v_tenant_id
    FROM public.entry_orders
    WHERE id = p_order_id;

    v_resultado_clean := LOWER(TRIM(p_resultado_revision));

    -- 2. Ejecutamos la actualización según las reglas de negocio
    UPDATE public.tenant_credits
    SET
        -- FUPAs: Se resta 1 si NO es reinspección (sin importar si es aprobada o rechazada).
        cupo_fupas = cupo_fupas - CASE 
            WHEN NOT COALESCE(p_es_reinspeccion, false) THEN 1 
            ELSE 0 
        END,

        -- Certificados: Se resta 1 si es Reinspección O si NO es reinspección y fue APROBADA.
        -- (Es decir: Solo NO se resta certificado cuando NO es reinspección y sale RECHAZADA).
        cupo_certificados = cupo_certificados - CASE 
            WHEN COALESCE(p_es_reinspeccion, false) THEN 1 
            WHEN v_resultado_clean = 'aprobado' THEN 1 
            ELSE 0 
        END
    WHERE tenant_id = v_tenant_id;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Operación cancelada: No se encontraron registros de créditos para este centro (Tenant: %).', v_tenant_id;
    END IF;
