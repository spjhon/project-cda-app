...
-- (Requerido porque cambió la estructura de retorno RETURNS TABLE)
...
-- 2. Recrear la función con la nueva estructura de columnas
...
RETURNS TABLE (
    ...
    -- 🌟 NUEVOS CAMPOS: CUPOS DISPONIBLES DEL TENANT
    cupo_fupas INTEGER,
...
BEGIN
    RETURN QUERY EXECUTE format('
        SELECT
            ...
            -- 🌟 PRESIONES DE LLANTAS EN JSONB
            COALESCE(tp.presiones, ''[]''::jsonb) AS presiones_llantas,

            -- 🌟 EXTRAER SALDOS/CUPOS DEL TENANT
        ...
        -- 🌟 CONEXIÓN PARA OBTENER CRÉDITOS DEL TENANT
        LEFT JOIN public.tenant_credits tc ON tc.tenant_id = o.tenant_id
