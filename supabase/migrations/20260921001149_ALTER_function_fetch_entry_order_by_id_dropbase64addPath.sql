DROP FUNCTION IF EXISTS public.fetch_entry_order_by_id(
    UUID,
    UUID
);
...
BEGIN
    ...
    SELECT
        ...
        -- Firma del funcionario: ruta de Storage
        o.funcionario_firma_path_snapshot AS funcionario_firma,
        ...
        -- Firma del director técnico: ruta de Storage
        o.director_tecnico_firma_path_snapshot AS director_tecnico_firma,
                    ...
                    'default_value',
                        CASE
                            WHEN ocr.value IS NOT NULL
                                THEN ocr.value
                            WHEN tc.is_special = TRUE
                                THEN tc.default_value
        ...
        -- Bloque Subconsulta: Firmas Dinámicas Complementarias
        (
            SELECT jsonb_agg(
                jsonb_build_object(
                    ...
                    'signature_path', os.signature_path,
                    'conditions', COALESCE(
                        (
                            SELECT jsonb_agg(
                                jsonb_build_object(
