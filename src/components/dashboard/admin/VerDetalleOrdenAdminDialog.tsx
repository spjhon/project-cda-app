...
export default function VerDetalleOrdenAdminDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* Solo mostramos el Trigger si el estado NO es controlado desde el padre */}
      {open === undefined && (
        <DialogTrigger
          render={
            <Button
      ...
      <DialogContent className="sm:max-w-4xl p-6 overflow-y-auto max-h-[90vh] bg-background">
        ...
        <DialogHeader className="flex flex-col items-center border-b border-slate-100 pb-4 text-center">
          ...
          <DialogDescription className="text-muted-foreground text-sm mt-1">
            Consola centralizada de solo lectura para la verificación de datos
            históricos e ISO 17020.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex flex-col gap-2.5 text-xs">
                ...
                <div className="flex justify-between border-b border-slate-100 pb-1.5">
                  ...
                  <span className="text-foreground font-medium capitalize">
                    {orden.vehiculo_tipo_snapshot} /{" "}
                    {orden.vehiculo_tipo_servicio_snapshot}
                  </span>
                </div>

                <div className="flex justify-between border-b border-slate-100 pb-1.5">
                  <span className="text-slate-400 font-medium">
                    Kilometraje en Entrada:
                  </span>
            ...
            <div className="flex flex-col bg-muted/30 p-4 rounded-xl border border-border/60 gap-4">
              ...
              <div className="flex flex-col gap-1 text-xs">
                ...
                <span className="text-muted-foreground text-[10px] uppercase">
                  {orden.propietario_tipo_documento}:{" "}
                  {orden.propietario_documento || "N/A"}
                </span>

                {orden.propietario_telefono && (
                  <span className="text-muted-foreground text-[10px]">
                    Teléfono: {orden.propietario_telefono}
                  </span>
                )}

                {orden.propietario_email && (
                  <span className="text-muted-foreground text-[10px] truncate">
                    Correo: {orden.propietario_email}
                  </span>
                )}

                {orden.propietario_direccion && (
                  <span className="text-muted-foreground text-[10px] truncate">
                    Dirección: {orden.propietario_direccion}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1 text-xs border-t border-border/60 pt-3">
                ...
                <span className="text-muted-foreground text-[10px] uppercase">
                  {orden.cliente_tipo_documento}:{" "}
                  {orden.cliente_documento || "N/A"}
                </span>
              ...
              <div className="flex flex-col gap-2.5 text-xs">
                ...
                <div className="flex justify-between border-b border-border pb-1.5">
                  ...
                  <span className="text-emerald-600 font-bold text-sm">
                    {formatCurrency(orden.rate_price_snapshot)}
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-2 pt-1">
                  ...
                  {orden.payments.length === 0 ? (
                    <span className="text-muted-foreground italic bg-muted px-2 py-1 rounded">
                      No hay pagos registrados
                    </span>
                  ) : (
                    <div className="flex flex-col gap-2">
                      {orden.payments.map((payment) => {
                        const esTarjeta =
                        ...
                        return (
                          <div
                            key={payment.id}
                            className="bg-muted/40 p-2.5 rounded-lg border border-border"
                          >
