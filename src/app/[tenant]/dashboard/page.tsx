...
> = {
  gerente: {
    label: "Administrador",
    description:
      "Gestión global, reportes financieros y control total del CDA.",
    path: "/dashboard/admin", // 👈 Controlado desde el objeto
  },
  recepcionista: {
    label: "Recepcionista",
    description:
      "Creación de órdenes de entrada, manejo de caja y flujo inicial.",
    path: "/dashboard/recepcionista",
  },
  aux_administrativo: {
    label: "Auxiliar Administrativo",
    description:
      "Gestión de documentos, control de archivos y soporte general.",
    path: "/dashboard/oficina",
  },
  director_tecnico: {
    label: "Director Técnico",
    description:
      "Control de calidad del SGC, aprobación e inspección bajo ISO 17020.",
...
export default function DashboardPage() {
  ...
  // 🌟 PASO 1: Extraemos solo el primer rol (si existe y es único)
  ...
  useEffect(() => {
    // 🌟 PASO 2: Dependemos únicamente del string 'singleRole'
    if (singleRole) {
      const targetPath =
        ROLE_CONFIG[singleRole]?.path || `/dashboard/${singleRole}`;
      router.replace(targetPath);
    }
  }, [singleRole, router]); // 👈 Dependencia limpia y primitiva
    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col items-center justify-center p-6">
      ...
      <div className="max-w-2xl w-full space-y-8">
        ...
        <div className="text-center space-y-2">
          <Badge
            variant="outline"
            className="px-3 py-1 border-primary/20 text-primary bg-primary/5 mb-2"
          >
            Panel de Acceso
          </Badge>
